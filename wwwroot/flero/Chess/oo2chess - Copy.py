...
##        else
    ...
    def CheckPlayer(self,pl,FideID,Country):
        Outp = db.session.query(ChessPlayer).filter(ChessPlayer.Name == pl).all()#.order_by(ChessSet.LastDate.desc()).one()
        if Outp == []:
            N = ChessPlayer(['',pl,'',Country,'','',FideID])
            db.session.add(N)
            db.session.commit()
            return(N)
        else:
    ...
    def CheckGame(self,Row):
        OneG = Row._mapping
        if 'White' not in OneG.keys() or 'Black' not in OneG.keys():
        ...
        if None in [SD, ED]:
            return [None, None], None, None
        IDE = self.CheckEvent(OneG['Event'], SD, ED, tformat)
        ...
        if SeedsOnly == True:
            # print('1',[IDW,IDB],SD,ED)
            return [IDW, IDB], SD, ED
        Moves = bytearray()
        board = game.board()
        try:
        ...
        try:
            ...
            for node in game.mainline():
                ...
                if m == 2:
                    break
            # print(firsttime)
            if ddt(OneG["Date"]) > datetime.date.today():
                TC = '15+10'
            elif firsttime[0] < 300.1 and firsttime[0] - firsttime[1] > 45.0:
                TC = 'Armageddon 5&4'
            ...
            TC = 'Unknown'
        # print('IDW',IDW)
        #self.CheckGame(IDW, IDB, IDE, SD, ED, OneG, Moves, TC, tformat, tournament2, NT)
        # print('2',[IDW,IDB],SD,ED)
        R = OneG['Round']
        GSeedsW = aliased(db.FirstSeeds)
        GSeedsB = aliased(db.FirstSeeds)
        #print('IDW',IDW)
        Outp = db.session.query(ChessGame).\
               join(GSeedsW,IDW.id == GSeedsW.c.player).\
               join(GSeedsB, and_(IDB.id == GSeedsB.c.player)).\
               join(ChessGame, and_(GSeedsW.c.encounter == ChessGame.id,GSeedsB.c.encounter == ChessGame.id)).\
               filter(ChessGame.StartDate <= ED).\
               filter(ChessGame.EndDate >= SD).\
               filter(ChessGame.PGNEvent == IDE.id).\
        ...
        if Outp == []:
            ...
            if NT != None:
                ...
                #print('GMoves',G.Moves)
                db.session.commit()
            else: #def __init__(self,Seeds,TimeControl,Date,EndDate,World,Round,Name='',PGNEvent=None,Moves=bytearray(),Result=None):
                N = ChessGame({0:[None,'First',IDW],1:[None,'First',IDB]},Timecontrol,SD,ED,self,R,'',IDE.id,Moves,Result)
            #print(N)
            #self.session.commit()
                db.session.add(N)
                db.session.commit()
                #IW = db.FirstSeeds.insert().values(encounter=N.id,player=IDW.id,spot=0)
                #IB = db.FirstSeeds.insert().values(encounter=N.id,player=IDB.id,spot=1)
            #print(IW)
    ...
    def AddSingleGametoDB(self,game,filename2,tformat,tournament2,NT,SeedsOnly):
        ...
        if len(Games) == 0:
            ...
            Qu = dbc.PGN.update().where(dbc.PGN.c.filename == filename2,
                                                 ...
                                                 dbc.PGN.c.Round == OneG['Round'],
                                                 dbc.PGN.c.White == OneG['White'],
                                                 dbc.PGN.c.Black == OneG['Black']).values(format = tformat,
                                                    tournament = tournament2,
                                                    filename = filename2,
                                                    Result = OneG['Result'],
                                                    Moves = str(game.mainline_moves()),
...
##        if yes: do nothing
##        if update: replace
##        if no: add game. create or update chessset. event tag name is used.

    def AddPGNtoDB(self,PGNfile,filename,tformat,tournament,NT,SeedsOnly,FD,LD):
        Pls = []
        while True:
            first_game = chess.pgn.read_game(PGNfile)
            if first_game == None:
                break
            #Pl,SD,ED = self.AddSingleGametoDB(first_game,filename,tformat,tournament,NT,SeedsOnly)
            self.AddSingleGametoDB(first_game,filename,tformat,tournament,NT,SeedsOnly)
            continue
            if SeedsOnly == True and Pl != [None,None]:
                for i in [Pl[0],Pl[1]]:
                    if i not in Pls:
                        Pls.append(i)
                FD = min(FD,SD)
    ...
    def CrawlPGNs(self):
        ...
        for fo in Formats:   
            ...
            for cg in CompGroup:
                ...
                SD = datetime.date(2099,1,1)
                ED = datetime.date(1699,1,1)
                if fo == 'Other' and cg == 'Other':
                    NT = None
                else:
                    #continue
                    T = db.session.query(ChessSet).filter(ChessSet.Type==fo,ChessSet.Name==cg).all()
                    if T == []:
                        Pls2 = []
                        for f in Files:
                            DB = open(Fold4+f)
                            Pls,SD,ED = self.AddPGNtoDB(DB,f,fo,cg,None,True,SD,ED)
                            for pl in Pls:
                                if pl not in Pls2:
                                    Pls2.append(pl)
                        Seeding = {}
                        for pl2 in range(len(Pls2)):
                            Seeding[pl2] = [None,'First',Pls2[pl2]]                       
                        NT = ChessSet(fo,Seeding,SD,ED,cg,self)
                        #print(NT)
                        #print(NT.Children)
                        db.session.add(NT)
                        db.session.commit()
                ...
                for f in Files:
                    print(fo,cg,f)
                    DB = open(Fold4+f)
                    Pls,SD,ED = self.AddPGNtoDB(DB,f,fo,cg,NT,False,SD,ED)
                    
                #ChessSet(fo,{},D,D,cg) #Type,Seeds,Date,EndDate,Name=''
                # In game, check players. 
                # If game does not exist yet: Create or update Chessset, Event tag name is used.
                # Chessset child of World
                # Games children of new Chessset
                # Chessset seeds empty
...
class ChessSet(oo3.Set):
    ...
    def __init__(self,Type,Seeds,Date,EndDate,Name,World): #split up
    ...
    #def propagate(self,Type,Seeds,Date,EndDate,Name,World):
        ...
        if Type == 'Meltwater Tournament':
            self.Children.append(ChessSet('Meltwater Prelims',self.Seeding,Date,Date+td(2),'Prelims',World))
            NSeeds = {}
            for i in range(8):
                NSeeds[i] = [self.Children[0],'Ranks',i]
            self.Children.append(ChessSet('Meltwater Knockout',NSeeds,Date+td(3),Date+td(8),'Knockout',World))
            ...
            for i in range(int(P/2)):
                self.Children.append(ChessSet('Meltwater KO Match',{0:self.Seeding[i],1:self.Seeding[7-i]},Date,Date+td(1),'Quarterfinal '+str(i+1),World))
            for i in range(int(P/4)):
                self.Children.append(ChessSet('Meltwater KO Match',{0:[self.Children[0+i],'Ranks',0],1:[self.Children[3-i],'Ranks',0]},Date+td(2),Date+td(3),'Semifinal '+str(i+1),World))
            self.Children.append(ChessSet('Meltwater KO Match',{0:[self.Children[4],'Ranks',0],1:[self.Children[5],'Ranks',0]},Date+td(4),Date+td(5),'Final',World))
            self.Children.append(ChessSet('Meltwater KO Match',{0:[self.Children[4],'Ranks',1],1:[self.Children[5],'Ranks',1]},Date+td(4),Date+td(5),'3rd place match',World))
        elif Type == 'Meltwater KO Match':
            self.TieBreakers = [0,1,2]
            self.Children.append(ChessSet('Meltwater KO Sets',self.Seeding,self.StartDate,self.EndDate,'Rapid',World))            
            self.Children.append(ChessSet('Meltwater 2 Blitz',self.Seeding,self.EndDate,self.EndDate,'Blitz tie-breaker',World))
            self.Children.append(ChessSet('Meltwater Armageddon',self.Seeding,self.EndDate,self.EndDate,'Armageddon tie-breaker',World))
            ...
            for i in range(2):
                self.Children.append(ChessSet('Meltwater KO Set',self.Seeding,Dts[i],Dts[i],'Set '+str(i+1),World))
            ...
            self.Children.append(ChessSet('MCI2020 Prelims',self.Seeding,Date,Date+td(12),'Prelims',World))
            NSeeds = {}
            for i in range(4):
                NSeeds[i] = [self.Children[0],'Ranks',i]
            self.Children.append(ChessSet('MCI2020 Final 4',NSeeds,Date+td(13),Date+td(15),'Final 4',World))
            ...
            for i in range(int(P/2)):
                self.Children.append(ChessSet('MCI 2020 KO Match',{0:self.Seeding[i],1:self.Seeding[3-i]},Date+td(i),Date+td(i),'Semifinal '+str(i+1),World))
            self.Children.append(ChessSet('MCI 2020 KO Match',{0:[self.Children[0],'Ranks',0],1:[self.Children[1],'Ranks',0]},Date+td(2),Date+td(2),'Final',World))
        elif Type == 'MCI 2020 KO Match':
            self.TieBreakers = [0,1,2,3]
            self.Children.append(ChessSet('Meltwater KO Set',self.Seeding,self.StartDate,self.EndDate,'Rapid',World))            
            self.Children.append(ChessSet('Meltwater 2 Blitz',self.Seeding,self.EndDate,self.EndDate,'Blitz tie-breaker',World))  
            self.Children.append(ChessSet('Meltwater 2 Blitz',self.Seeding,self.EndDate,self.EndDate,'Blitz tie-breaker',World))
            self.Children.append(ChessSet('Meltwater Armageddon',self.Seeding,self.EndDate,self.EndDate,'Armageddon 5&4',World))
            self.HMatrix = numpy.array([[0,0,0,0]])
            self.AMatrix = numpy.array([[1,1,1,1]])           
        elif Type == 'MCI2020 Match':
            self.TieBreakers = [0,1]
            self.Children.append(ChessSet('Meltwater KO Set',self.Seeding,self.StartDate,self.EndDate,'Rapid',World))
            self.Children.append(ChessSet('Meltwater Armageddon',self.Seeding,self.StartDate,self.EndDate,'Armageddon 5&4',World)) 
            self.HMatrix = numpy.array([[0,0]])
            self.AMatrix = numpy.array([[1,1]])
        elif Type == 'Meltwater Tournament 2022':
            self.Children.append(ChessSet('Meltwater Prelims 2022',self.Seeding,Date,Date+td(3),'Prelims',World))
            NSeeds = {}
            for i in range(8):
                NSeeds[i] = [self.Children[0],'Ranks',i]
            self.Children.append(ChessSet('Meltwater Knockout 2022',NSeeds,Date+td(4),Date+td(7),'Knockout',World))
            ...
            for i in range(int(P/2)):
                self.Children.append(ChessSet('Meltwater KO Match 2022',{0:self.Seeding[i],1:self.Seeding[7-i]},Date,Date,'Quarterfinal '+str(i+1),World))
            for i in range(int(P/4)):
                self.Children.append(ChessSet('Meltwater KO Match 2022',{0:[self.Children[0+i],'Ranks',0],1:[self.Children[3-i],'Ranks',0]},Date+td(1),Date+td(1),'Semifinal '+str(i+1),World))
            self.Children.append(ChessSet('Meltwater KO Match',{0:[self.Children[4],'Ranks',0],1:[self.Children[5],'Ranks',0]},Date+td(2),Date+td(3),'Final',World))
    ...
    def ChiBerger(self,P,S,GoS,TimeControl,Type=''):
        ...
        for i in R:
            if GoS == 'ChessGame':
                ...
                self.Children.append(ChessSet(Type,{0:self.Seeding[self.HMatrix[0,i]],1:self.Seeding[self.AMatrix[0,i]]},self.StartDate,self.EndDate,'Match '+str(Rounds[i])+'/'+str(len(Rounds)),self.World))
